...
import { ArrowRight, Award, FileCheck2, Wallet } from "lucide-react";
import { Badge, ButtonLink, Disclosure, Eyebrow, SourceLink, VerificationBadge } from "@/components/ui";
...
const STAGE_FILTERS = Object.entries(STAGE_LABELS);
...
export default async function ScholarshipsPage({ searchParams }: { searchParams: Promise<{ stage?: string }> }) {
  ...
  const stage = params.stage && params.stage in STAGE_LABELS ? params.stage : null;
  ...
  return <div className="cb-container cb-page">
    ...
    <nav aria-label="Scholarship study level" className="my-6 flex flex-wrap gap-2"><Link href="/scholarships" aria-current={!stage ? "page" : undefined} className={`cb-button border px-4 py-2 text-sm ${!stage ? "cb-button-primary" : "border-ink-200 bg-white text-ink-700 hover:bg-butter/40"}`}>All schemes</Link>{STAGE_FILTERS.map(([id, label]) => <Link href={`/scholarships?stage=${id}`} key={id} aria-current={stage === id ? "page" : undefined} className={`cb-button border px-4 py-2 text-sm ${stage === id ? "cb-button-primary" : "border-ink-200 bg-white text-ink-700 hover:bg-butter/40"}`}>{label}</Link>)}</nav>
    ...
    <div className="grid items-start gap-5 xl:grid-cols-2 2xl:grid-cols-3">{ordered.map((scholarship, i) => <article key={scholarship.slug} id={scholarship.slug} className="overflow-hidden rounded-2xl border border-ink-200 bg-white">
      ...
      <div className="p-5"><VerificationBadge status={scholarship.verificationStatus} lastVerifiedAt={scholarship.lastVerifiedAt} />
        ...
        <div className="mt-4 space-y-2"><Disclosure summary="Who can apply?"><p>{scholarship.eligibility ?? "CareerBridge could not verify this information from an authoritative source."}</p><div className="mt-3 flex flex-wrap gap-1.5">{(scholarship.appliesToStage ?? []).map((s) => <Badge key={s}>{STAGE_LABELS[s] ?? s}</Badge>)}</div></Disclosure><Disclosure summary="Documents to keep ready"><ul className="list-disc space-y-1 pl-5 text-sm leading-relaxed text-ink-700">{(scholarship.documents ?? []).map((document) => <li key={document}>{document}</li>)}</ul><p className="mt-3 text-xs text-ink-500">The current notification may ask for different documents.</p></Disclosure><Disclosure summary="Source & verification"><VerificationBadge status={scholarship.verificationStatus} lastVerifiedAt={scholarship.lastVerifiedAt} /><div className="mt-3"><SourceLink href={scholarship.sourceUrl ?? scholarship.officialUrl} variant="inline">Open source website</SourceLink></div></Disclosure></div>
      </div>
    </article>)}</div>
