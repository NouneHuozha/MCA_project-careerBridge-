...
export default async function ExamsPage() {
  ...
  return <div className="cb-container cb-page">
    ...
    <div className="grid items-start gap-5 xl:grid-cols-2 2xl:grid-cols-3">{exams.map((exam, i) => <article key={exam.slug} id={exam.slug} className="overflow-hidden rounded-2xl border border-ink-200 bg-white">
      ...
      <div className="p-5"><VerificationBadge status={exam.verificationStatus} lastVerifiedAt={exam.lastVerifiedAt} /><div className="mt-4 flex flex-wrap gap-1.5">{exam.category && <Badge>{exam.category.replace(/_/g, " ")}</Badge>}{exam.scope && <Badge>{exam.scope}</Badge>}{exam.state && <Badge>{exam.state}</Badge>}{exam.examMode && <Badge>{exam.examMode}</Badge>}{exam.status && <Badge tone={exam.status === "active" ? "green" : "neutral"}>{exam.status}</Badge>}</div><p className="mt-4 text-sm leading-relaxed text-ink-600">{(exam.appliesTo ?? []).join(" · ")}</p>
