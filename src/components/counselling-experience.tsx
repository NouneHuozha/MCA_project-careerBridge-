...
export function CounsellingExperience({ initial, focusKey }: { initial: State; focusKey?: string }) {
  ...
  const questionNumber = question ? Math.min(total, done + 1) : total;
  const sectionLabel = question ? state.sections.find((section) => section.key === question.section)?.label : null;
  ...
  return <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_300px] 2xl:grid-cols-[minmax(0,1fr)_380px]">
    <section className="cb-counselling-shell" aria-label="Guided conversation">
      <header className="border-b border-forest-200 bg-mint/65 px-4 py-3 sm:px-5">
        <div className="flex items-center justify-between gap-3"><span className="flex items-center gap-2 text-sm font-bold text-forest-800"><MessageCircle aria-hidden className="h-4 w-4" />Understanding you</span><span className="text-xs font-semibold text-forest-800">{done} of {total} answered</span></div>
        <div className="mt-2 flex items-center justify-between gap-3"><ProgressDots total={total} current={done} label="Questions answered" /><button type="button" disabled={pending} onClick={() => confirmReset.current?.showModal()} className="flex min-h-8 items-center gap-1.5 text-xs font-semibold text-forest-700 underline underline-offset-4"><RotateCcw aria-hidden className="h-3 w-3" />Restart</button></div>
      </header>
      ...
      <div ref={composer} className="cb-counselling-composer cb-scroll p-4 sm:p-5">
        {error && <div className="mb-3"><Callout tone="amber"><p role="alert">{error}</p></Callout></div>}
          ...
          <legend className="text-base font-semibold leading-snug text-ink-900 sm:text-lg">{question.prompt}</legend>
          {question.helper && <p className="mt-2 max-w-[62ch] text-sm leading-relaxed text-ink-500">{question.helper}</p>}
          {question.maxSelections && <p className="mt-3 text-xs font-semibold text-ink-500">Choose up to {question.maxSelections} · {selected.length} selected</p>}
          {question.answerType === "text" ? <><label className="sr-only" htmlFor="counselling-text">Your answer</label><textarea id="counselling-text" rows={3} maxLength={600} value={text} onChange={(e) => setText(e.target.value)} placeholder="In your own words…" className="mt-3 w-full rounded-xl border border-ink-200 bg-canvas p-3 text-sm leading-relaxed" /></> : <div key={question.key} className="mt-3 grid gap-2 sm:grid-cols-2">{(question.options ?? []).map((option) => <button type="button" key={option.value} aria-pressed={selected.includes(option.value)} onClick={() => choose(option.value)} className="cb-choice min-h-[48px] justify-start text-left">{selected.includes(option.value) && <Check aria-hidden className="h-3.5 w-3.5 shrink-0" />}{option.label}{option.hint && <span className="block text-[11px] font-normal opacity-75">{option.hint}</span>}</button>)}</div>}
          ...
          <div className="mt-3 flex items-center justify-between gap-3 text-xs text-ink-500">{lastAnswered && !focusKey ? <Link href={`/counselling?edit=${lastAnswered.key}`} className="flex items-center gap-1 font-semibold text-forest-700 underline underline-offset-4"><ArrowLeft aria-hidden className="h-3 w-3" />Edit previous answer</Link> : <span>You can change this later.</span>}<Link href="/profile" className="font-semibold text-forest-700 underline underline-offset-4">Save & leave</Link></div>
